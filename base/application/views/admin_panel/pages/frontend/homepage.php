...
<ul id="sortable1SK" class="connectedSortable">
                    <?php  $num = 0; foreach ($featured_fields as $item) {?>
                        <?php $flag = true; 
                        ...
                        if ($item->id == $hitem->field_id && $item->title == $hitem->title) $flag = false; } ?>
                            ...
                            <input type="hidden" name=item[<?= $num ?>][is_cat] value='<?php if (isset($item->parent_title)) echo 0; else echo 1; ?>'>
                            </li>
                    <?php $num++; } } ?>
...
<ul id="sortable2SK" class="connectedSortable">
                    <?php foreach ($homepage_fields as $item) { ?>
                        ...
                        <?php } else { ?><li <?php if(!$item->is_cat) echo 'style="background-color: #eaecee"' ?> class="btn btn-secondary m-btn m-btn--air m-btn--custom"><?php echo $item->title;}?>
...
<script>
$("#smenu_homepage").addClass("m-menu__item m-menu__item--submenu m-menu__item--open m-menu__item--expanded");
var num = <?= $num ?>
</script>
